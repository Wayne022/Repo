   ...
   public int findMinimum( int column )
   {
	   int count = 0;
	    ...
	    for (int row = 0; row < intArray.length; row++) {
	        ...
	        for (int column1 = 0; column1 < intArray[row].length; column1++) {
	            int value = 0;
				// If we find the target value, increment the counter
	            if (intArray[row][column1] == value) {
	                count++;
	            }

	            // Animate after checking each element
	            animate(row, column1, count);
	        }
	    }

	    // Return how many times the value was found
	    return count;
   }
